...
fn solve(a: Vec<i64>, b: Vec<i64>, c: Vec<i64>) -> i64
{
...
fn main_rs(args: Vec<String>) -> i32
{
    ...
    while cnt > 0
    {
        ...
        bstr.pop();
        targetstr.pop();
        let avec: Vec<i64> = astr[12..].split(", Y+").map(|x|x.parse().unwrap()).collect();
        let bvec: Vec<i64> = bstr[12..].split(", Y+").map(|x|x.parse().unwrap()).collect();
        let tvec: Vec<i64> = targetstr[9..].split(", Y=").map(|x|10000000000000+x.parse::<i64>().unwrap()).collect();
